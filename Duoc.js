...
// Chia câu hỏi thành 5 phần động theo tổng số câu hỏi
const PARTS_COUNT = 5;
const perPart = Math.ceil(questionsData.length / PARTS_COUNT);
const questions = Array.from({ length: PARTS_COUNT }, (_, i) =>
  questionsData.slice(i * perPart, Math.min((i + 1) * perPart, questionsData.length))
);

// Cập nhật nhãn nút menu theo dải câu hỏi thực tế (nếu tồn tại trên trang)
function updateMenuLabels() {
  const total = questionsData.length;
  const per = Math.ceil(total / PARTS_COUNT);

  for (let i = 0; i < PARTS_COUNT; i++) {
    const start = i * per + 1;
    const end = Math.min((i + 1) * per, total);
    const btn = document.querySelector(`.menu-btn[data-part="${i + 1}"]`);
    if (btn) {
      btn.textContent = `Phần ${i + 1} (Câu ${start}-${end})`;
    }
  }

  const allBtn = document.querySelector('.menu-btn[data-part="all"]');
  if (allBtn) {
    allBtn.textContent = `Tất Cả (${total} câu)`;
  }
}

// Thử cập nhật ngay khi script tải (an toàn nếu script được nhúng sau HTML)
updateMenuLabels();
