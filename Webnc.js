  ...
  if (!quizQuestions.length) {
    ...
    if (quizPart)
      quizPart.textContent =
        currentPart === 0 ? "Tất Cả" : `Phần ${currentPart}`;
...
function renderQuiz() {
  ...
  if (questionContent)
    questionContent.textContent = q.isMultiple
      ? q.question + " (Chọn nhiều đáp án)"
      : q.question;
...
function showResult() {
  ...
  let rank =
    percent === 100
      ...
      : "Luyện tập thêm nhé!";
  if (resultSummary)
    resultSummary.innerHTML = `<div><b>Số câu đúng:</b> ${correct} / ${total}</div><div><b>Phần trăm đúng:</b> ${percent}%</div><div><b>Đánh giá:</b> ${rank}</div>`;
}
...
if (restartBtn)
  restartBtn.addEventListener("click", () => {
    let selectedQuestions;
    if (currentPart === 0) selectedQuestions = questions.flat();
    else
